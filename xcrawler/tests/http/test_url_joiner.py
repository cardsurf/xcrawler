...
class TestUrlJoiner(unittest.TestCase):
    ...
    @mock.patch.object(UrlJoiner, 'join_protocol_domain_to_url')
    def test_join_protocol_domain_to_urls(self, mock_join_protocol_domain_to_url):
        ...
        mock_join_protocol_domain_to_url.return_value = "http://test.com/link/to/example_page.html"
        result = self.url_joiner.join_protocol_domain_to_urls(mock_protocol_domain, mock_urls)
        self.assertEquals(mock_join_protocol_domain_to_url.call_count, len(mock_urls))
    ...
    def test_join_protocol_domain_to_url(self, mock_urljoin_function):
        mock_protocol_domain = "http://test.com"
        mock_url = ".link/to/example_page.html"
        mock_urljoin_function.return_value = ["http://test.com/link/to/example_page.html"]
        result = self.url_joiner.join_protocol_domain_to_url(mock_protocol_domain, mock_url)
